      ...
      fluidRow(
        column(
          ...
          div(
            ...
            h3("Friendly Match"),
            p(
              "Anyone can submit a friendly result. No admin password is required."
            ),
            div(
...
server <- function(input, output, session) {
  ...
  observeEvent(input$submit_friendly_match, {
    req(input$friendly_player_a, input$friendly_player_b)
    ...
    tryCatch({
      # Always start from the latest Dropbox copy so a public submission
      # does not overwrite a match submitted by somebody else moments earlier.
      latest_players <- tryCatch(
        load_players(),
        error = function(e) players()
      )

      latest_matches <- tryCatch(
        load_matches(),
        error = function(e) matches()
      )

      latest_players <- rename_players_to_photo_names(
        normalize_players(latest_players)
      )

      latest_matches <- rename_matches_to_photo_names(
        normalize_matches(latest_matches)
      )

      if (!pa %in% latest_players$player || !pb %in% latest_players$player) {
        stop("One of the selected players no longer exists.")
      }
      ...
      rebuilt <- recalculate_all(
        bind_rows(latest_matches, raw),
        latest_players
      )

      # Persist matches + ranking + history to Dropbox.
      save_everything(latest_players, rebuilt)

      # Verify the newly submitted match exists in Dropbox.
      verify_matches <- load_matches()

      verified_row <- verify_matches %>%
        filter(match_id == new_id, stage == "Friendly")

      if (nrow(verified_row) != 1) {
        stop("Dropbox verification failed after saving the friendly match.")
      }

      players(latest_players)
      matches(recalculate_all(verify_matches, latest_players))

      result_text <- if (ga > gb) {
        paste0(pa, " +2 • ", pb, " -2")
      } else if (gb > ga) {
        paste0(pb, " +2 • ", pa, " -2")
      } else {
        paste0("Draw: ", pa, " 0 • ", pb, " 0")
      }
      ...
      friendly_status(
        list(
          type = "success",
          text = paste0(
            "Friendly result saved successfully. ",
            result_text
          )
        )
      )

      # Reset only the score boxes; keep selected players.
      updateNumericInput(
        session,
        "friendly_goals_a",
        value = 0
  ...
  output$friendly_submit_status <- renderUI({
    ...
    if (is.null(s)) {
      return(
        tags$p(
          style = "color:#667085;",
          "Submitted friendly matches immediately affect the live rating."
        )
      )
    }
