...
# ============================================================
# PLAYER PHOTO MAPPING
# ============================================================

player_photo_map <- c(
  "Naser" = "naser.jpg",
  "Tanha" = "morteza shafaye.jpg",
  "Rezgar" = "photo_2025-04-29_16-07-43.jpg",
  "Abolfazl" = "abolfazl askari.jpg",
  "Sepehri" = "mohammad sepehri.jpg",
  "Sia" = "siawash.jpg",
  "Tohid" = "tohid.jpg",
  "Firoozi" = "amir firuzi.jpg",
  "Ali Af" = "ali farahani.jpg",
  "Ghajar" = "ami ghajar.jpg",
  "Mortazavi" = "mohammad mortazavi.jpg",
  "Vahid" = "vahid.jpg",
  "Ahmad" = "ahmad.jpg",
  "Mori" = "jamal.jpg",
  "M Fadaei" = "mehdi fadaei.jpg",
  "Mhdi" = "mehdi.jpg",
  "A Aa" = "abbas.jpg",
  "Reza Shokri" = "reza shokri.jpg",
  "Mojtaba Elyasi" = "mojtaba.jpg",
  "Seyed Behdad Arshahi" = "seed behdad.jpg",
  "Mehdi Mameshli" = "mehdi mamashali.jpg"
)

player_photo <- function(player_name) {
  file <- unname(player_photo_map[player_name])
  if (length(file) == 0 || is.na(file) || !nzchar(file)) return(NULL)
...
dropbox_upload_matches <- function(x){
  ...
  # Every save now refreshes ALL Dropbox backup files.
  save_all_backups(normalize_matches(x), load_players())
  invisible(TRUE)
}
...
dropbox_upload_csv <- function(x, path) {
  ...
  if (resp_status(resp) >= 300) {
    stop(paste("Dropbox upload failed for", path))
  }
  ...
  for (p in players$player) {
    ...
    if (nrow(pm) > 0) {
      for (i in seq_len(nrow(pm))) {
        ...
        is_a <- r$player_a == p
        opponent <- if (is_a) r$player_b else r$player_a
        ...
        out[[idx]] <- tibble(
          ...
          opponent = opponent,
          result = result,
          goals_for = as.integer(gf),
          goals_against = as.integer(ga),
          participation_bonus = as.numeric(bonus),
          rating_change = as.numeric(delta),
          rating_after = as.numeric(current)
        )
        idx <- idx + 1L
      }
    }
...
save_all_backups <- function(matches, players) {
  ranking_now <- build_ranking(players, matches)

  player_stats <- ranking_now %>%
    select(
      player,
      rating,
      matches,
  ...
  rating_history <- build_rating_history_all(players, matches)

  # Local copies for convenience when running on a desktop.
  write_csv(normalize_matches(matches), "matches_local.csv")
  write_csv(ranking_now, "ranking_local.csv")
  write_csv(player_stats, "players_stats_local.csv")
  write_csv(rating_history, "rating_history_local.csv")

  # Persistent Dropbox backups.
  dropbox_upload_csv(normalize_matches(matches), DROPBOX_MATCHES_PATH)
  dropbox_upload_csv(ranking_now, DROPBOX_RANKING_PATH)
  dropbox_upload_csv(player_stats, DROPBOX_PLAYERS_STATS_PATH)
  dropbox_upload_csv(rating_history, DROPBOX_RATING_HISTORY_PATH)
...
server <- function(input,output,session){
  ...
  output$profile_header <- renderUI({
    ...
    fluidRow(
      column(
        ...
        if (!is.null(photo)) {
          tags$img(
            src = photo,
            alt = s$player,
            style = "width:120px;height:120px;object-fit:cover;border-radius:50%;border:3px solid #e5e7eb;box-shadow:0 2px 8px rgba(0,0,0,.15);"
          )
        } else {
          div(
            style = "width:120px;height:120px;border-radius:50%;background:#e5e7eb;display:flex;align-items:center;justify-content:center;font-size:42px;font-weight:700;color:#6b7280;",
            substr(s$player, 1, 1)
          )
        }
      ),
      column(
        10,
        div(
          style = "font-size:30px;font-weight:800;margin-top:24px;",
          s$player,
          tags$br(),
          tags$span(
  ...
  output$admin_panel<-renderUI({if(!admin())div(class="cardx",h3("Admin Login"),passwordInput("admin_password","Password"),actionButton("admin_login","Login",class="btn-primary"),br(),br(),div(class="admin-no","Match entry is locked.")) else tagList(
    ...
    div(
      ...
      h3("Dropbox Backups"),
      tags$ul(
        tags$li("matches.csv — official match database"),
        tags$li("ranking.csv — current live ranking"),
        tags$li("players_stats.csv — current player statistics"),
        tags$li("rating_history.csv — rating history after every match")
      ),
  ...
  output$edit_panel <- renderUI({
    req(admin(), selected_id())

    r <- matches() %>%
      filter(match_id == selected_id())
    ...
    div(
      ...
      actionButton("update_match", "Update Selected Match", class = "btn-warning"),
      tags$span(" "),
      actionButton("delete_match", "Delete Selected Match", class = "btn-danger")
    )
  })
