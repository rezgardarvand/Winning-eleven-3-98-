...
make_flexible_draw <- function(ranking, participants, cup_id) {
  ...
  seed_count <- min(8L, nrow(selected_rank))
...
load_tournament_archive_from_dropbox <- function() {
  ...
  # IMPORTANT:
  # If tournament_draws.csv exists, it is ALWAYS authoritative,
  # even when it contains zero rows after deleting the last saved draw.
  # This prevents deleted draws from being resurrected from the old
  # tournament_draw.csv compatibility file after refresh/restart.
  if (!is.null(arch)) {
    if (nrow(arch) == 0) {
      return(arch)
    }
  ...
  # One-time backward compatibility only when the archive file
  # does not exist at all.
  single <- tryCatch(
    dropbox_download_csv(
      DROPBOX_TOURNAMENT_PATH,
      quiet = FALSE
    ),
    error = function(e) NULL
  )

  if (!is.null(single) && nrow(single) > 0) {
    migrated <- normalize_tournament_archive(single)

    # Create the official archive immediately so future loads
    # never rely on the legacy single-draw file.
    try(
      save_tournament_archive(migrated),
      silent = TRUE
    )
...
server <- function(input, output, session) {
  ...
  saved_archive <- load_tournament_archive_from_dropbox()
  ...
  observeEvent(input$save_draw, {
    ...
    tryCatch({
      ...
      updateSelectInput(
        session,
        "admin_saved_cup",
        choices = unique(new_arch$cup_id),
        selected = d$cup_id[1]
      )
