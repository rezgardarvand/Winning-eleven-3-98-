...
render_bracket_tree <- function(draw) {
  ...
  rounds <- build_bracket_rounds(draw)
  rnames <- names(rounds)

  render_match <- function(m, round_index) {
    advance_note <- NULL

    if (round_index == 1) {
      if (isTRUE(m$bye_a) && !isTRUE(m$bye_b)) {
        advance_note <- paste0(m$player_b, " advances automatically")
      } else if (isTRUE(m$bye_b) && !isTRUE(m$bye_a)) {
        advance_note <- paste0(m$player_a, " advances automatically")
      }
    }

    div(
      class = "tree-match",
      div(class = "tree-match-label", paste0("Match ", m$match_no)),
      div(
        class = paste("tree-player", if (isTRUE(m$bye_a)) "tree-bye" else ""),
        span(m$player_a)
      ),
      div(
        class = paste("tree-player", if (isTRUE(m$bye_b)) "tree-bye" else ""),
        span(m$player_b)
      ),
      if (!is.null(advance_note)) {
        div(class = "tree-next", advance_note)
      }
    )
  }

  round_columns <- lapply(seq_along(rounds), function(ri) {
    games <- rounds[[ri]]

    div(
      class = "bracket-round",
      div(class = "bracket-round-title", rnames[ri]),
      div(
        class = "bracket-round-games",
        lapply(games, function(m) render_match(m, ri))
      )
    )
  })
  ...
  champion_col <- div(
    class = "bracket-round",
    div(class = "bracket-round-title", "Champion"),
    div(
      class = "bracket-round-games",
      div(
        class = "tree-champion",
