...
DROPBOX_BACKUP_MATCHES_PATH <- "/backup_last_matches.csv"
DROPBOX_BACKUP_PLAYERS_PATH <- "/backup_last_players.csv"
DROPBOX_BACKUP_RANKING_PATH <- "/backup_last_ranking.csv"
...
save_everything <- function(players, matches) {
  ...
  # Last-known-good mirrors for emergency recovery.
  dropbox_upload_csv(matches, DROPBOX_BACKUP_MATCHES_PATH)
  dropbox_upload_csv(players, DROPBOX_BACKUP_PLAYERS_PATH)
  dropbox_upload_csv(ranking_now, DROPBOX_BACKUP_RANKING_PATH)
    ...
    tabPanel(
      ...
      div(
        ...
        tags$ul(
          ...
          tags$li("There is no Reset system."),
...
server <- function(input, output, session) {
      ...
      tabsetPanel(
        ...
        tabPanel(
          ...
          div(
            ...
            h3("Dropbox Recovery"),
            p(
              "Use this if the app ever shows incorrect or empty data after a restart, shutdown or republish. ",
              "The app will reload the permanent player and match files from Dropbox and rebuild all ratings and statistics in match_id order."
            ),
            uiOutput("dropbox_recovery_status"),
            br(),
            actionButton(
            ...
            tags$small(
              style = "color:#667085;",
              "This does not delete matches and does not reset ratings. matches.csv remains the source of truth."
            )
          )
        ),
  ...
  # ----------------------------
  # DROPBOX BACKUP / RECOVERY
  # ----------------------------

  recovery_last_result <- reactiveVal("Ready. No recovery has been run in this session.")

  output$dropbox_recovery_status <- renderUI({
    tagList(
      tags$b("Source of truth: Dropbox matches.csv"),
      tags$br(),
      tags$span(
        style = "color:#667085;",
  ...
  observeEvent(input$restore_from_dropbox, {
    req(admin_logged_in())

    showNotification(
      "Reading the latest saved files from Dropbox...",
      type = "message",
      duration = 3
    )

    tryCatch({
      # Load the latest permanent files directly from Dropbox.
      cloud_players <- dropbox_download_csv(DROPBOX_PLAYERS_PATH)
      cloud_matches <- dropbox_download_csv(DROPBOX_MATCHES_PATH)

      if (is.null(cloud_players) || nrow(cloud_players) == 0) {
        stop("players.csv could not be read from Dropbox.")
      }

      p <- rename_players_to_photo_names(normalize_players(cloud_players))

      if (is.null(cloud_matches)) {
        m_raw <- empty_matches
      } else {
        m_raw <- rename_matches_to_photo_names(normalize_matches(cloud_matches))
      }

      # Rebuild ALL match ratings sequentially from match_id 1 onward.
      ...
      # Regenerate all derived Dropbox files from the permanent matches.
      save_everything(p, rebuilt)
      ...
      recovery_last_result(
        paste0(
          ...
          " matches and ",
          nrow(p),
          " players loaded from Dropbox. ",
          "Ranking and all statistics were rebuilt successfully."
        )
      )

      showNotification(
        "Dropbox recovery completed successfully.",
        type = "message",
        duration = 7
      )
      ...
      recovery_last_result(
        paste("Recovery failed:", conditionMessage(e))
      )
  ...
  observeEvent(input$restore_last_good_backup, {
    req(admin_logged_in())

    showNotification(
      "Loading last known good Dropbox backup...",
      type = "message",
      duration = 3
    )

    tryCatch({
      bp <- dropbox_download_csv(DROPBOX_BACKUP_PLAYERS_PATH)
      bm <- dropbox_download_csv(DROPBOX_BACKUP_MATCHES_PATH)

      if (is.null(bp) || nrow(bp) == 0) {
        stop("No backup_last_players.csv file was found.")
      }
      if (is.null(bm)) {
        stop("No backup_last_matches.csv file was found.")
      }

      p <- rename_players_to_photo_names(normalize_players(bp))
      m_raw <- rename_matches_to_photo_names(normalize_matches(bm))
      rebuilt <- recalculate_all(m_raw, p)
      ...
      showNotification(
        "Last good backup restored successfully.",
        type = "message",
        duration = 7
      )
      ...
      recovery_last_result(
        paste("Backup recovery failed:", conditionMessage(e))
      )
      showNotification(
        paste("Backup recovery failed:", conditionMessage(e)),
        type = "error",
