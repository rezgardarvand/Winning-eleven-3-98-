...
# Player names are standardized to match the photo filenames in www/.
# Uncertain players are left unchanged.
# ============================================================

canonical_player_name <- c(
...
canonicalize_name <- function(x) {
  x <- as.character(x)
  hit <- x %in% names(canonical_player_name)
  x[hit] <- unname(canonical_player_name[x[hit]])
  x
}

canonicalize_players <- function(p) {
  p$player <- canonicalize_name(p$player)
  p
}

canonicalize_matches <- function(m) {
  m$player_a <- canonicalize_name(m$player_a)
  m$player_b <- canonicalize_name(m$player_b)
  m
}


# ============================================================
# PLAYER PHOTOS - AUTOMATIC
# The app player name is intended to match the photo filename stem.
# ============================================================
...
load_players <- function() {
  ...
  if (!is.null(cloud)) return(canonicalize_players(normalize_players(cloud)))

  if (file.exists(PLAYERS_FILE)) {
    return(canonicalize_players(normalize_players(read_csv(PLAYERS_FILE, show_col_types = FALSE))))
  }

  canonicalize_players(normalize_players(NULL))
}

load_matches <- function() {
  cloud <- dropbox_download_csv(DROPBOX_MATCHES_PATH)
  if (!is.null(cloud)) return(canonicalize_matches(normalize_matches(cloud)))

  if (file.exists(LOCAL_MATCHES_FILE)) {
    return(canonicalize_matches(normalize_matches(read_csv(LOCAL_MATCHES_FILE, show_col_types = FALSE))))
  }
...
save_everything <- function(players, matches) {
  players <- canonicalize_players(normalize_players(players))
  matches <- canonicalize_matches(normalize_matches(matches))
