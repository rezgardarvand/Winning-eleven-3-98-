...
server <- function(input, output, session) {
  ...
  saved_archive <- tryCatch(
    ...
    error = function(e) {
      message("Tournament archive startup read failed: ", conditionMessage(e))
      NULL
    }
  )
  ...
  rankdat <- reactive({
    tryCatch(
      ...
      error = function(e) {
        message("Ranking rebuild failed: ", conditionMessage(e))
        tibble()
      }
    )
  })
  ...
  observe({
    ...
    tryCatch({
      cloud_players <- tryCatch(
        load_players(),
        error = function(e) {
          message("Dropbox players refresh failed: ", conditionMessage(e))
          NULL
        }
      )
      ...
      cloud_matches <- tryCatch(
        load_matches(),
        error = function(e) {
          message("Dropbox matches refresh failed: ", conditionMessage(e))
          NULL
        }
      )
      ...
      if (!is.null(cloud_matches)) {
        rebuilt_matches <- tryCatch(
          recalculate_all(cloud_matches, players()),
          error = function(e) {
            message("Match recalculation refresh failed: ", conditionMessage(e))
            NULL
          }
        )

        if (!is.null(rebuilt_matches)) {
          matches(rebuilt_matches)
        }
      }
    }, error = function(e) {
      message("Background refresh safely ignored: ", conditionMessage(e))
    })
  })
