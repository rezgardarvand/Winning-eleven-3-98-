...
build_ranking <- function(players, matches, reset_state) {
  ...
  # Only matches after the reset boundary affect the current Rating.
  # All matches still count for historical match/win/goal statistics.
  rating_matches <- rating_era_matches(matches, reset_state)

  out <- lapply(players$player, function(p) {
    base <- players %>% filter(player == p)
    pm <- matches %>% filter(player_a == p | player_b == p)
    rpm <- rating_matches %>% filter(player_a == p | player_b == p)

    rc <- if (nrow(rpm)) {
      sum(ifelse(rpm$player_a == p, rpm$rating_change_a, rpm$rating_change_b), na.rm = TRUE)
    } else 0

    bonus <- if (nrow(rpm)) {
      sum(ifelse(rpm$player_a == p, rpm$participation_a, rpm$participation_b), na.rm = TRUE)
    } else 0
    ...
    tibble(
      player = p,
      rating = RATING_BASELINE + rc + bonus,
    ...
    tabPanel(
      ...
      div(
        ...
        tags$ul(
          ...
          tags$li("Only matches recorded after the reset point affect the current Rating."),
