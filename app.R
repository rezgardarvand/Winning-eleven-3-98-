...
render_bracket_tree <- function(draw) {
  ...
  match_box <- function(r, side = "left") {
    ...
    # A player-vs-BYE row is not an actual match.
    # It means this player qualified directly to the next round.
    # Keep an invisible spacer only for bracket alignment.
    if (
      xor(a == "BYE", b == "BYE")
    ) {
      return(
  ...
  make_next_pairs <- function(ids, prefix) {
    ...
    lapply(seq_along(groups), function(i) {
      ...
      feeder_label <- function(match_id) {
        r <- d %>% filter(match_no == match_id)

        if (nrow(r) != 1) {
          return(paste0("Winner Match ", match_id))
        }

        # BYE-vs-BYE means this feeder contributes no opponent.
        if (
          identical(as.character(r$player_a[1]), "BYE") &&
          identical(as.character(r$player_b[1]), "BYE")
        ) {
          return("BYE")
        }

        # A normal player-vs-BYE feeder contributes the real player directly.
        if (identical(as.character(r$player_a[1]), "BYE")) {
          return(as.character(r$player_b[1]))
        }

        if (identical(as.character(r$player_b[1]), "BYE")) {
          return(as.character(r$player_a[1]))
        }

        paste0("Winner Match ", match_id)
      }

      a <- feeder_label(x[1])
      b <- if (length(x) > 1) feeder_label(x[2]) else "BYE"
  ...
  left_col2 <- div(
    ...
    div(
      class = "mb-title",
      if (identical(as.character(d$round[1]), "Preliminary")) {
        round_name_from_bracket(as.integer(d$bracket_size[1]) / 2L)
      } else {
        "Quarterfinal"
      }
    ),
