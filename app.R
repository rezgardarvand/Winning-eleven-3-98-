...
RATING_BASELINE <- 300
RATING_RESET_PATH <- "/rating_reset.csv"
...
# ============================================================
# RATING RESET / NEW ERA
# Old matches remain for statistics and archive.
# Only matches with match_id > reset_after_match_id affect the new Rating.
# ============================================================

default_reset_state <- function(matches) {
  cutoff <- if (is.null(matches) || nrow(matches) == 0) {
    0L
  } else {
    as.integer(max(matches$match_id, na.rm = TRUE))
  }

  tibble(
    reset_after_match_id = cutoff,
    reset_date = as.character(Sys.Date()),
    baseline_rating = RATING_BASELINE
  )
}

load_reset_state <- function(matches) {
  cloud <- tryCatch(
    dropbox_download_csv(RATING_RESET_PATH),
    error = function(e) NULL
  )

  if (!is.null(cloud) &&
      nrow(cloud) >= 1 &&
      "reset_after_match_id" %in% names(cloud)) {
    return(
      tibble(
        reset_after_match_id = as.integer(cloud$reset_after_match_id[1]),
        reset_date = as.character(cloud$reset_date[1]),
        baseline_rating = RATING_BASELINE
      )
    )
  }

  # First run of this reset-enabled version:
  # freeze the current last match as the reset boundary.
  state <- default_reset_state(matches)

  try(
    dropbox_upload_csv(state, RATING_RESET_PATH),
    silent = TRUE
  )
...
save_reset_state <- function(state) {
  dropbox_upload_csv(state, RATING_RESET_PATH)
  invisible(TRUE)
}

rating_era_matches <- function(matches, reset_state) {
  cutoff <- as.integer(reset_state$reset_after_match_id[1])

  if (nrow(matches) == 0) return(empty_matches)

  matches %>%
    filter(match_id > cutoff) %>%
    arrange(match_id)
}
...
recalculate_all <- function(matches, players, reset_state) {
  ...
  rebuilt_current <- empty_matches

  for (i in seq_len(nrow(current_raw))) {
    r <- current_raw[i, ]
    ...
    calc <- calculate_match(
      ...
      r$stage, r$cup,
      players, rebuilt_current
    )

    rebuilt_current <- bind_rows(
      rebuilt_current,
      tibble(
        match_id = r$match_id,
        date = r$date,
  ...
  bind_rows(archived, rebuilt_current) %>%
    arrange(match_id)
}

build_ranking <- function(players, matches, reset_state) {
  ...
  # NEW ERA ONLY:
  # all current ranking statistics and rating changes are based only
  # on matches after the reset boundary.
  current_matches <- rating_era_matches(matches, reset_state)
  ...
  out <- lapply(players$player, function(p) {
    ...
    gf <- if (nrow(pm)) {
      sum(
        ifelse(pm$player_a == p, pm$goals_a, pm$goals_b),
        na.rm = TRUE
      )
    } else 0
    ...
    ga <- if (nrow(pm)) {
      sum(
        ifelse(pm$player_a == p, pm$goals_b, pm$goals_a),
        na.rm = TRUE
      )
    } else 0

    total_points <- rc + bonus
    current_rating <- RATING_BASELINE + total_points
    mpg <- nrow(pm)
    ...
    tibble(
      ...
      points_per_game = ifelse(mpg == 0, 0, total_points / mpg)
    )
  })
...
build_rating_history_all <- function(players, matches, reset_state) {
  out <- list()
  k <- 1L
  matches <- rating_era_matches(matches, reset_state)
...
save_everything <- function(players, matches, reset_state) {
  ...
  ranking_now <- build_ranking(players, matches, reset_state)
  stats_now <- ranking_now %>%
    select(
      player, rating, matches, wins, losses,
      goals_for, goals_against, goal_difference, win_rate
    )
  history_now <- build_rating_history_all(players, matches, reset_state)
  ...
  navbarPage(
    ...
    tabPanel(
      ...
      div(
        ...
        h3("Ranking Rules"),
        tags$ul(
          tags$li(tags$b("Rating baseline: 300"), " for every player at the current reset point."),
          tags$li("After reset, Matches, Wins, Losses, Goals, Goal Difference and Win Rate all start from zero."),
          tags$li("Only matches recorded after the reset point affect the current Ranking and current statistics."),
          tags$li(tags$b("Total Points"), " = current Rating − 300."),
          tags$li(tags$b("Points/Game"), " = Total Points divided by current-era Matches."),
          tags$li(tags$b("+11 participation points"), " once per player for each Cup ID after the reset."),
...
server <- function(input, output, session) {
  ...
  initial_players <- load_players()
  initial_matches <- load_matches()
  initial_reset <- load_reset_state(initial_matches)

  players <- reactiveVal(initial_players)
  reset_state <- reactiveVal(initial_reset)
  matches <- reactiveVal(
    recalculate_all(initial_matches, initial_players, initial_reset)
  )
  ...
  rankdat <- reactive({
    build_ranking(players(), matches(), reset_state())
  })
  ...
  observe({
    ...
    if (!is.null(cloud_matches)) {
      matches(recalculate_all(cloud_matches, players(), reset_state()))
    }
  })
  ...
  observe({
    ...
    current_era <- rating_era_matches(matches(), reset_state())
    cups <- sort(unique(current_era$cup))
  ...
  output$m_points_per_game <- renderText({
    round(stats()$points_per_game, 2)
  })
  ...
  player_match_data <- reactive({
    ...
    rating_era_matches(matches(), reset_state()) %>%
      filter(player_a == p | player_b == p) %>%
      arrange(match_id) %>%
      mutate(
        is_a = player_a == p,
  ...
  output$rating_chart <- renderPlot({
    ...
    hist <- build_rating_history_all(players(), matches(), reset_state()) %>%
  ...
  output$public_matches <- renderDT({
    m <- rating_era_matches(matches(), reset_state())
      ...
      tabsetPanel(
        ...
        tabPanel(
          ...
          div(
            ...
            h3("New Rating Era"),
            p("Current baseline is 300 for every player. Old matches stay in the archive and continue to count for match/win/goal statistics, but they do not affect the new Rating."),
            uiOutput("rating_reset_status"),
            br(),
            actionButton(
              "reset_rating_now",
              "Reset All Ratings to 300 Now",
              class = "btn-danger"
            )
          )
        ),
  ...
  observeEvent(input$add_match, {
    ...
    rebuilt <- recalculate_all(bind_rows(m, raw), players(), reset_state())

    tryCatch({
      save_everything(players(), rebuilt, reset_state())
      matches(rebuilt)
  ...
  observeEvent(input$update_match, {
    ...
    rebuilt <- recalculate_all(m, players(), reset_state())
  ...
  observeEvent(input$add_player, {
    ...
    tryCatch({
      save_everything(p, matches(), reset_state())
      players(p)
  ...
  observeEvent(input$update_player, {
    ...
    rebuilt <- recalculate_all(m, p, reset_state())

    tryCatch({
      save_everything(p, rebuilt, reset_state())
      players(p)
      matches(rebuilt)
  ...
  output$rating_reset_status <- renderUI({
    s <- reset_state()
    cutoff <- as.integer(s$reset_after_match_id[1])
    ...
    div(
      ...
      tags$span(
        style = "color:#667085;",
        paste0(
          "Current reset boundary: match #",
          cutoff,
          ". Only matches after this ID affect Rating."
        )
      )
    )
  })

  observeEvent(input$reset_rating_now, {
    req(admin_logged_in())

    current_matches <- matches()
    cutoff <- if (nrow(current_matches) == 0) {
      0L
    } else {
      as.integer(max(current_matches$match_id, na.rm = TRUE))
    }
    ...
    tryCatch({
      ...
      # Existing matches remain unchanged as archive.
      # Ranking becomes exactly 300 for every player immediately.
      # Keep old matches in storage, but current-era statistics become zero.
      matches(current_matches)
      save_everything(players(), current_matches, new_state)

      showNotification(
        "All Ratings reset to 300. New matches will now build Rating sequentially from this point.",
        type = "message",
        duration = 8
      )
    }, error = function(e) {
      showNotification(
        paste("Rating reset failed:", conditionMessage(e)),
        type = "error",
        duration = NULL
      )
    })
