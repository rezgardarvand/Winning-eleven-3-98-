...
make_flexible_draw <- function(ranking, participants, cup_id) {
  ...
  # Number that can go directly to the next full knockout round.
  #
  # 10 players:
  #   bracket_size = 16
  #   direct_count = 6
  #   preliminary players = 4 -> 2 preliminary matches
  #
  # After those 2 matches there are exactly 8 players.
  direct_count <- as.integer(bracket_size - n_players)
  preliminary_match_count <- as.integer(n_players - n_matches)

  first_round <- if (direct_count > 0) {
    "Preliminary"
  } else {
    round_name_from_bracket(bracket_size)
  }

  direct_players <- selected_rank %>%
    slice_head(n = direct_count) %>%
    arrange(cup_seed)

  preliminary_players <- selected_rank %>%
    filter(cup_seed > direct_count) %>%
    pull(player)

  # Lower-ranked players who must play the preliminary round are randomized.
  if (length(preliminary_players) > 0) {
    preliminary_players <- sample(
      preliminary_players,
      length(preliminary_players),
      replace = FALSE
    )
  }
  ...
  # Standard protected placement for direct qualifiers.
  ...
  if (n_matches >= 8) {
    ...
    seed_to_match <- setNames(
      seq_len(n_matches),
      as.character(seq_len(n_matches))
    )
  }

  # Direct qualifiers occupy one feeder slot and have no actual first-round game.
  if (nrow(direct_players) > 0) {
    for (i in seq_len(nrow(direct_players))) {
      s <- direct_players$cup_seed[i]

      if (!as.character(s) %in% names(seed_to_match)) {
        next
      }

      mi <- as.integer(seed_to_match[as.character(s)])

      slots$player_a[mi] <- direct_players$player[i]
      slots$player_b[mi] <- "BYE"
      slots$seed_a[mi] <- s
      slots$seed_b[mi] <- NA_integer_
    }
  }

  # Every remaining completely empty feeder is a REAL preliminary match.
  empty_matches <- which(
    is.na(slots$player_a) &
    is.na(slots$player_b)
  )

  if (length(empty_matches) != preliminary_match_count) {
    stop(
      paste0(
        "Internal draw error: expected ",
        preliminary_match_count,
        " preliminary matches but found ",
        length(empty_matches),
        "."
      )
    )
  }

  if (length(preliminary_players) != preliminary_match_count * 2L) {
    stop("Internal draw error: preliminary participant count is incorrect.")
  }

  if (preliminary_match_count > 0) {
    k <- 1L

    for (mi in empty_matches) {
      slots$player_a[mi] <- preliminary_players[k]
      slots$player_b[mi] <- preliminary_players[k + 1L]
      k <- k + 2L
    }
  }

  # A power-of-two draw has no direct qualifiers; all slots must now be filled.
  slots$player_a[is.na(slots$player_a)] <- "BYE"
  slots$player_b[is.na(slots$player_b)] <- "BYE"

  # There must never be a meaningless BYE-vs-BYE "match".
  if (any(slots$player_a == "BYE" & slots$player_b == "BYE")) {
    stop("Draw generation failed: an empty BYE-vs-BYE match was created.")
  }
  ...
  # Every selected player must appear exactly once.
  ...
  duplicated_in_draw <- unique(
    drawn_players[duplicated(drawn_players)]
  )
...
render_bracket_tree <- function(draw, results = NULL) {
  ...
  cup_id_value <- as.character(d$cup_id[1])

  result_data <- if (is.null(results)) {
    empty_matches
  } else {
    rename_matches_to_photo_names(
      normalize_matches(results)
    ) %>%
      filter(
        .data$cup == cup_id_value,
        .data$stage != "Friendly",
  ...
  is_placeholder <- function(x) {
    is.na(x) ||
      ...
      grepl("^Winner ", as.character(x)) ||
      identical(as.character(x), "BYE")
  }

  find_pair_result <- function(a, b) {
    a <- as.character(a)
    b <- as.character(b)
    ...
    # Knockout rounds cannot advance on a draw.
    z <- z %>%
  ...
  result_info <- function(a, b) {
    r <- find_pair_result(a, b)
    ...
    pa <- as.character(r$player_a[1])
    pb <- as.character(r$player_b[1])
    ga <- as.integer(r$goals_a[1])
    gb <- as.integer(r$goals_b[1])

    winner <- if (ga > gb) pa else pb
    loser <- if (ga > gb) pb else pa

    # Return scores in the requested a/b orientation.
    score_a <- if (a == pa) ga else gb
    score_b <- if (b == pb) gb else ga
  ...
  player_line <- function(
    player,
    seed = NA_integer_,
    state = c("normal", "winner", "eliminated"),
    score = NULL,
    advanced = FALSE
  ) {
    ...
    cls <- if (identical(player, "BYE")) {
      "mb-player mb-bye"
    } else if (state == "eliminated") {
      "mb-player mb-player-eliminated"
    } else if (isTRUE(advanced)) {
      "mb-player mb-player-advanced"
    } else {
      "mb-player"
    }

    # Winner styling is expressed mainly in the NEXT round as
    # blue + arrow. Eliminated players remain red and struck through.
    style <- switch(
      state,
    ...
    div(
      ...
      style = style,
      if (!is.na(seed)) {
        span(class = "mb-seed", paste0("#", seed))
      },
      if (isTRUE(advanced) && !identical(player, "BYE")) {
        span(class = "mb-advance-arrow", "→")
      },
      span(class = "mb-player-name", player),
      if (!is.null(score) && !is.na(score)) {
        span(class = "mb-score", score)
      },
      if (state == "eliminated") {
        span(class = "mb-eliminated-label", "Eliminated")
      }
    )
  }

  first_feeder <- function(match_id) {
    r <- d %>% filter(match_no == match_id)
    if (nrow(r) != 1) {
      return(
        list(
          entrant = paste0("Winner Match ", match_id),
          resolved = FALSE,
          result = NULL
        )
      )
    ...
    a <- as.character(r$player_a[1])
    b <- as.character(r$player_b[1])

    if (a == "BYE" && b != "BYE") {
      return(list(entrant = b, resolved = TRUE, result = NULL))
    }

    if (b == "BYE" && a != "BYE") {
      return(list(entrant = a, resolved = TRUE, result = NULL))
    }

    if (a == "BYE" && b == "BYE") {
      return(list(entrant = "BYE", resolved = TRUE, result = NULL))
    }

    info <- result_info(a, b)

    if (isTRUE(info$played)) {
      return(
        list(
          entrant = info$winner,
          resolved = TRUE,
  ...
  match_box <- function(r, side = "left") {
    a <- as.character(r$player_a)
    b <- as.character(r$player_b)

    # Direct qualifier:
    # display the player-vs-BYE card so the first-round bracket remains
    # visually complete, while progression logic still treats it as a BYE.
    info <- result_info(a, b)
    ...
    div(
      class = paste(
        "mb-match",
        if (side == "left") "mb-left-match" else "mb-right-match"
      ),
      ...
      player_line(
        ...
        r$seed_a,
        a_state,
        if (isTRUE(info$played)) info$score_a else NULL
      ),
      player_line(
        b,
        r$seed_b,
        b_state,
        if (isTRUE(info$played)) info$score_b else NULL
      )
    )
  }

  build_round_pairs <- function(feeder_ids, prefix) {
    if (length(feeder_ids) == 0) return(list())

    groups <- split(
      feeder_ids,
      ceiling(seq_along(feeder_ids) / 2)
    )

    lapply(seq_along(groups), function(i) {
      ids <- groups[[i]]

      fa <- first_feeder(ids[1])
      fb <- if (length(ids) > 1) {
        first_feeder(ids[2])
      } else {
        list(entrant = "BYE", resolved = TRUE)
      }
      ...
      list(
        label = paste0(prefix, " ", i),
        a = a,
        b = b,
        a_resolved = isTRUE(fa$resolved),
        b_resolved = isTRUE(fb$resolved),
        result = info,
        winner = winner,
        resolved = resolved
      )
  ...
  round_box <- function(x, side = "left") {
    info <- x$result

    a_state <- "normal"
    b_state <- "normal"

    if (!is.null(info) && isTRUE(info$played)) {
      if (identical(x$a, info$winner)) {
        a_state <- "winner"
        b_state <- "eliminated"
      } else {
        a_state <- "eliminated"
    ...
    # A real resolved entrant with no result yet in this round has
    # advanced from the previous round. Highlight it blue + arrow.
    a_advanced <- (
      is.null(info) &&
      isTRUE(if (!is.null(x$a_resolved)) x$a_resolved else FALSE) &&
      !is_placeholder(x$a)
    )

    b_advanced <- (
      is.null(info) &&
      isTRUE(if (!is.null(x$b_resolved)) x$b_resolved else FALSE) &&
      !is_placeholder(x$b)
    )
    ...
    div(
      ...
      player_line(
        x$a,
        state = a_state,
        score = if (!is.null(info) && isTRUE(info$played)) {
          info$score_a
        } else {
          NULL
        },
  ...
  right_first <- if (length(right_idx) > 0) {
    d[right_idx, , drop = FALSE]
  } else {
    d[0, , drop = FALSE]
  }

  left_round2 <- build_round_pairs(
    left_first$match_no,
    "QF"
  )

  right_round2 <- build_round_pairs(
    right_first$match_no,
    "QF"
  )

  build_side_semifinal <- function(round2, label = "SF") {
    if (length(round2) == 0) {
      return(
        list(
          label = label,
    ...
    a_obj <- round2[[1]]
    b_obj <- if (length(round2) >= 2) {
      round2[[2]]
    } else {
      NULL
    }

    a <- if (isTRUE(a_obj$resolved)) {
      a_obj$winner
    } else {
      "Winner QF 1"
    }

    b <- if (!is.null(b_obj) && isTRUE(b_obj$resolved)) {
      b_obj$winner
    ...
    } else if (b == "BYE" && !is_placeholder(a)) {
      winner <- a
      resolved <- TRUE
    }
    ...
    list(
      ...
      a = a,
      b = b,
      a_resolved = isTRUE(a_obj$resolved),
      b_resolved = if (!is.null(b_obj)) isTRUE(b_obj$resolved) else TRUE,
      result = info,
      winner = winner,
      resolved = resolved
    )
  ...
  left_sf <- build_side_semifinal(
    left_round2,
    "SF"
  )

  right_sf <- build_side_semifinal(
    right_round2,
    "SF"
  )

  final_a <- if (isTRUE(left_sf$resolved)) {
    left_sf$winner
  } else {
    "Winner Left SF"
  }

  final_b <- if (isTRUE(right_sf$resolved)) {
    right_sf$winner
  } else {
    "Winner Right SF"
  }
  ...
  if (
    !is_placeholder(final_a) &&
    !is_placeholder(final_b)
  ) {
    final_info <- result_info(final_a, final_b)

    if (isTRUE(final_info$played)) {
      champion <- final_info$winner
    }
  }

  first_round_title <- as.character(d$round[1])

  next_round_title <- if (
    identical(first_round_title, "Preliminary")
  ) {
    round_name_from_bracket(
      as.integer(d$bracket_size[1]) / 2L
    )
  } else {
    round_name_from_bracket(
      as.integer(d$bracket_size[1]) / 2L
  ...
  # Display labels for the full 16-slot bracket.
  if (length(left_round2) >= 1) left_round2[[1]]$label <- "QF 1"
  if (length(left_round2) >= 2) left_round2[[2]]$label <- "QF 2"
  if (length(right_round2) >= 1) right_round2[[1]]$label <- "QF 3"
  if (length(right_round2) >= 2) right_round2[[2]]$label <- "QF 4"

  left_sf$label <- "SF 1"
  right_sf$label <- "SF 2"

  # Final states are unchanged; only the presentation below is new.
  final_a_state <- "normal"
  final_b_state <- "normal"

  if (!is.null(final_info) && isTRUE(final_info$played)) {
    if (identical(final_a, final_info$winner)) {
      final_a_state <- "winner"
      final_b_state <- "eliminated"
    } else {
      final_a_state <- "eliminated"
      final_b_state <- "winner"
    }
  }

  slot <- function(content, cls, left, top, width) {
    div(
      class = paste("mb-abs-slot", cls),
      style = paste0(
        "left:", left, "px;",
        "top:", top, "px;",
        "width:", width, "px;"
  ...
  round_title <- function(label, left, width) {
    div(
      class = "mb-abs-round-title",
      style = paste0(
        "left:", left, "px;",
        "top:8px;",
        "width:", width, "px;"
  ...
  # Fixed, reference-style canvas coordinates.
  # Left: R16 -> QF -> SF -> FINAL
  # Right is the exact mirror.
  r16_y <- c(78, 228, 378, 528)
  qf_y  <- c(153, 453)
  sf_y  <- 303

  left_first_cards <- lapply(
    seq_len(nrow(left_first)),
    function(i) {
      slot(
        match_box(left_first[i, ], "left"),
        "mb-stage-r16 mb-side-left",
        10,
        r16_y[i],
        180
      )
  ...
  right_first_cards <- lapply(
    seq_len(nrow(right_first)),
    function(i) {
      slot(
        match_box(right_first[i, ], "right"),
        "mb-stage-r16 mb-side-right",
        1280,
        r16_y[i],
        180
      )
  ...
  left_qf_cards <- lapply(
    seq_along(left_round2),
    function(i) {
      slot(
        round_box(left_round2[[i]], "left"),
        "mb-stage-qf mb-side-left",
        245,
        qf_y[i],
        190
      )
  ...
  right_qf_cards <- lapply(
    seq_along(right_round2),
    function(i) {
      slot(
        round_box(right_round2[[i]], "right"),
        "mb-stage-qf mb-side-right",
        1035,
        qf_y[i],
        190
      )
  ...
  left_sf_card <- slot(
    round_box(left_sf, "left"),
    "mb-stage-sf mb-side-left",
    470,
    sf_y,
    160
  )
  ...
  right_sf_card <- slot(
    round_box(right_sf, "right"),
    "mb-stage-sf mb-side-right",
    840,
    sf_y,
    160
  )
  ...
  final_stack <- div(
    ...
    div(
      ...
      player_line(
        final_a,
        state = final_a_state,
        score = if (!is.null(final_info) && isTRUE(final_info$played)) {
          final_info$score_a
        } else {
          NULL
        },
      ...
      player_line(
        ...
        advanced = (
          is.null(final_info) &&
          isTRUE(right_sf$resolved) &&
          !is_placeholder(final_b)
        )
      ),
      div(
        class = "mb-champion",
        if (!is.na(champion) && nzchar(champion)) {
          tagList(
            span(class = "mb-champion-label", "🏆 CHAMPION"),
            span(class = "mb-champion-name", champion)
          )
        } else {
          span(class = "mb-champion-label", "CHAMPION")
        }
  ...
  # SVG connector network: real L-shaped blue paths behind the cards.
  connector_svg <- div(
    class = "mb-svg-layer",
    HTML(
      '<svg viewBox="0 0 1470 700" preserveAspectRatio="none" aria-hidden="true">
        <defs>
          <marker id="mbArrow" markerWidth="8" markerHeight="8" refX="7" refY="4" orient="auto" markerUnits="strokeWidth">
            <path d="M0,0 L8,4 L0,8 z" fill="#1378e8"></path>
          </marker>
        </defs>

        <!-- LEFT: Round of 16 -> Quarterfinal -->
        <path class="mb-path" d="M190 117 H215 V192 H245" marker-end="url(#mbArrow)"></path>
        <path class="mb-path" d="M190 267 H215 V192 H245" marker-end="url(#mbArrow)"></path>
        <path class="mb-path" d="M190 417 H215 V492 H245" marker-end="url(#mbArrow)"></path>
        <path class="mb-path" d="M190 567 H215 V492 H245" marker-end="url(#mbArrow)"></path>

        <!-- LEFT: Quarterfinal -> Semifinal -->
        <path class="mb-path" d="M435 192 H452 V342 H470" marker-end="url(#mbArrow)"></path>
        <path class="mb-path" d="M435 492 H452 V342 H470" marker-end="url(#mbArrow)"></path>

        <!-- LEFT: Semifinal -> Final -->
        <path class="mb-path" d="M630 342 H655" marker-end="url(#mbArrow)"></path>

        <!-- RIGHT: Round of 16 -> Quarterfinal -->
        <path class="mb-path" d="M1280 117 H1255 V192 H1225" marker-end="url(#mbArrow)"></path>
        <path class="mb-path" d="M1280 267 H1255 V192 H1225" marker-end="url(#mbArrow)"></path>
        <path class="mb-path" d="M1280 417 H1255 V492 H1225" marker-end="url(#mbArrow)"></path>
        <path class="mb-path" d="M1280 567 H1255 V492 H1225" marker-end="url(#mbArrow)"></path>

        <!-- RIGHT: Quarterfinal -> Semifinal -->
        <path class="mb-path" d="M1035 192 H1018 V342 H1000" marker-end="url(#mbArrow)"></path>
        <path class="mb-path" d="M1035 492 H1018 V342 H1000" marker-end="url(#mbArrow)"></path>

        <!-- RIGHT: Semifinal -> Final -->
        <path class="mb-path" d="M840 342 H815" marker-end="url(#mbArrow)"></path>
      </svg>'
    )
  )
  ...
  div(
    ...
    div(
      class = "mb-reference-toolbar",
      div(
        class = "mb-reference-title",
        "Tournament Bracket"
      ),
      div(
        class = "mb-reference-status",
        "Live results automatically advance winners"
      )
    ),
    div(
      class = "modern-bracket mb-reference-canvas",
      connector_svg,

      round_title(first_round_title, 10, 180),
      round_title(next_round_title, 245, 190),
      round_title("Semifinal", 470, 160),
      round_title("Semifinal", 840, 160),
      round_title(next_round_title, 1035, 190),
      round_title(first_round_title, 1280, 180),
    ...
    div(
      class = "mb-legend",
      span(
        class = "mb-legend-item",
        span(class = "mb-legend-winner-box"),
        "Qualified (Winner)"
      ),
      span(
        class = "mb-legend-item",
        span(class = "mb-legend-eliminated-line"),
        "Eliminated"
      ),
      span(
        class = "mb-legend-item",
        span(class = "mb-legend-arrow", "→"),
...
ui <- fluidPage(
  tags$head(
      ...
      tags$script(HTML("
        window.downloadTournamentBracketHD = async function(filename){
  const source = document.querySelector('.modern-bracket-wrap');

  if(!source){
    alert('Tournament bracket is not visible.');
    return;
  }
  ...
  stage.style.top = '0';
  stage.style.width = '1518px';
  stage.style.background = '#ffffff';
  stage.style.zIndex = '-9999';
  document.body.appendChild(stage);

  const clone = source.cloneNode(true);
  clone.classList.add('mb-export-copy');
  clone.style.width = '1518px';
  clone.style.minWidth = '1518px';
  clone.style.maxWidth = '1518px';
  ...
  try{
    ...
    link.download = filename || 'tournament_bracket_HD.png';
    ...
    console.error(err);
    alert('Could not create the bracket image.');
  }finally{
    stage.remove();
  }
};
...
.modern-bracket{
display:grid;
  grid-template-columns: 185px 205px 185px 165px 185px 205px 185px;
  gap:34px;
  min-width:1450px;
  align-items:stretch;
  background:#ffffff;
...
width:auto;
  ...
  border:0;
  box-shadow:none;
  filter:drop-shadow(0 3px 4px rgba(15,23,42,.16));
}
.mb-final{
width:150px;
  text-align:center;
...
.mb-player-eliminated{
color:#d92d20 !important;
  background:#ffffff !important;
  text-decoration-line:line-through !important;
  text-decoration-color:#ef2b2d !important;
  text-decoration-thickness:2px !important;
  opacity:1;
  font-weight:700 !important;
}
.bracket-download-bar{
  display:flex;
...
/* ==========================================================
   FINAL REFERENCE BRACKET THEME
   This block intentionally comes LAST so the dark app theme
   cannot override the tournament bracket.
   ========================================================== */
...
.modern-bracket,
.mb-export-copy .modern-bracket{
  display:grid !important;
  grid-template-columns:185px 205px 185px 170px 185px 205px 185px !important;
...
/* Real blue advancement arrows BETWEEN boxes. */
.mb-left-match::after{
  content:'→' !important;
  position:absolute !important;
...
.mb-left-match::before{
  display:none !important;
}

.mb-right-match::after{
  content:'←' !important;
  position:absolute !important;
  left:-34px !important;
  top:50% !important;
...
.mb-cup{
  ...
  line-height:1 !important;
  filter:drop-shadow(0 3px 4px rgba(15,23,42,.15)) !important;
}
...
.mb-champion{
  ...
  border:1px solid #f5d38d !important;
  border-radius:8px !important;
  opacity:1 !important;
  font-size:12px !important;
  font-weight:900 !important;
...
.mb-legend-winner-box{
  width:16px !important;
  height:16px !important;
  display:inline-block !important;
  background:#dceeff !important;
  border:1px solid #68aefc !important;
  border-radius:3px !important;
}

.mb-legend-eliminated-line{
  width:24px !important;
  display:inline-block !important;
  border-top:3px solid #ef2b2d !important;
}
...
/* Export copy is ALWAYS white regardless of the app's dark theme. */
.mb-export-stage,
.mb-export-copy,
.mb-export-copy *,
.mb-export-copy .modern-bracket,
.mb-export-copy .mb-col{
...
.mb-export-copy .mb-match,
.mb-export-copy .mb-final,
.mb-export-copy .mb-player,
.mb-export-copy .mb-matchno,
.mb-export-copy .mb-legend{
  background:#ffffff !important;
}

.mb-export-copy .mb-player-advanced{
  background:#eef6ff !important;
  color:#0866d9 !important;
}

.mb-export-copy .mb-player-eliminated{
  color:#d92d20 !important;
}
...
/* ==========================================================
   EXACT REFERENCE BRACKET LAYOUT — ABSOLUTE + SVG CONNECTORS
   ========================================================== */
...
.modern-bracket.mb-reference-canvas{
  position:relative !important;
...
.mb-player-eliminated .mb-player-name{
  color:#d92d20 !important;
  text-decoration-line:line-through !important;
  text-decoration-color:#ef2b2d !important;
  text-decoration-thickness:2px !important;
...
.mb-champion{
  ...
  background:#fff8e8 !important;
  color:#c97800 !important;
  border-top:1px solid #f5d38d !important;
...
.mb-legend-arrow{
  color:#1378e8 !important;
  font-size:20px !important;
  font-weight:900 !important;
  line-height:1 !important;
}

/* Export: always capture exactly this white full canvas. */
.mb-export-copy,
.mb-export-copy .modern-bracket-wrap,
.mb-export-copy .mb-reference-canvas,
.mb-export-copy .mb-match,
.mb-export-copy .mb-player,
.mb-export-copy .mb-final,
.mb-export-copy .mb-matchno,
...
rebuild_draw_fields <- function(d, ranking) {
  ...
  seed_count <- if (participant_count < bracket_size) {
    min(
      8L,
      n_matches,
      as.integer(bracket_size - participant_count)
    )
  } else {
    min(8L, n_matches, participant_count)
  }
...
server <- function(input, output, session) {
  ranking_refresh_tick <- reactiveVal(0L)
  ...
  initial_players <- tryCatch(
    rename_players_to_photo_names(normalize_players(load_players())),
    error = function(e) rename_players_to_photo_names(normalize_players(NULL))
  )

  initial_matches_raw <- tryCatch(
    rename_matches_to_photo_names(normalize_matches(load_matches())),
    error = function(e) empty_matches
  )

  initial_matches <- if (nrow(initial_matches_raw) > 0) {
    # Recalculate once at startup so old/legacy rows are normalized.
    recalculate_all(initial_matches_raw, initial_players)
  } else {
    initial_matches_raw
  }

  players <- reactiveVal(initial_players)
  matches <- reactiveVal(initial_matches)

  # Last known-good in-memory snapshots.
  # These prevent a transient Dropbox/network refresh from blanking Ranking.
  last_good_players <- reactiveVal(initial_players)
  last_good_matches <- reactiveVal(initial_matches)
  ...
  rankdat <- reactive({
    # Explicit invalidation signal so Ranking redraws immediately
    # after a successful match save, without browser refresh.
    ranking_refresh_tick()
  ...
  observe({
    ...
    if (
      !is.null(cloud_players) &&
      nrow(cloud_players) > 0 &&
      all(c("player", "base_rating") %in% names(cloud_players)) &&
      all(nzchar(trimws(cloud_players$player)))
    ) {
      ...
      if (!identical(
        ...
        last_good_players(cloud_players)
      }
    }

    if (
      !is.null(cloud_matches) &&
      all(
        c(
          "match_id","player_a","player_b",
          "rating_change_a","rating_change_b"
        ) %in% names(cloud_matches)
      )
    ) {
      ...
      if (!identical(
        ...
        # matches.csv already contains calculated rating fields.
        # Keep a last-good copy before/after sync.
        matches(cloud_matches)
        last_good_matches(cloud_matches)
      }
    }
  })
  ...
  force_ranking_refresh <- function() {
    ranking_refresh_tick(isolate(ranking_refresh_tick()) + 1L)
    invisible(TRUE)
  }
  ...
  observeEvent(
    ranking_refresh_tick(),
    {
      # renderDT is reactive already; this explicit invalidation ensures
      # the browser receives the updated widget immediately.
      session$onFlushed(function() {
        try({
          proxy <- DT::dataTableProxy("ranking_table", session = session)
          dat <- rankdat()
  ...
  output$ranking_table <- renderDT({
    ...
    if (nrow(dat) == 0) {
      fallback_players <- last_good_players()
      fallback_matches <- last_good_matches()

      if (!is.null(fallback_players) && nrow(fallback_players) > 0) {
        dat <- build_ranking(fallback_players, fallback_matches)
      }
    }
    ...
    if (nrow(dat) == 0) {
      return(
        datatable(
          data.frame(Message = "Ranking data is temporarily unavailable. Please refresh once."),
          rownames = FALSE
        )
      )
    }
  ...
  observeEvent(input$confirm_friendly_match, {
    ...
    tryCatch({
      ...
      last_good_players(latest_players)
      last_good_matches(updated_matches)
      force_ranking_refresh()
      player_pins(latest_pins)
  ...
  output$tournament_public <- renderUI({
    ...
    tagList(
      ...
      div(
        class = "cardx",
        div(
          class = "bracket-download-bar",
          tags$button(
            id = "download_bracket_hd",
            type = "button",
            class = "btn btn-primary",
            onclick = "downloadTournamentBracketHD();",
            "⬇ Download Bracket HD"
          )
        ),
        div(
          id = "tournament-bracket-capture",
          tryCatch(
            render_bracket_tree(d, matches()),
            error = function(e) {
              tags$p(
                style = "color:#d92d20;",
  ...
  output$admin_matches <- renderDT({
    ...
    # Use the last known-good copy if live matches() is temporarily empty.
    ...
    if (is.null(m) || nrow(m) == 0) {
      return(
        datatable(
          data.frame(Message = "No matches."),
          rownames = FALSE
        )
      )
    }

    m <- rename_matches_to_photo_names(
      normalize_matches(m)
    )
  ...
  observeEvent(input$admin_matches_rows_selected, {
    ...
    if (length(i) == 1) {
      ...
      ordered <- rename_matches_to_photo_names(
        normalize_matches(m)
      ) %>%
        arrange(desc(match_id))
  ...
  output$edit_match_panel <- renderUI({
    ...
    r <- m %>% filter(match_id == selected_match_id())
    req(nrow(r) == 1)
