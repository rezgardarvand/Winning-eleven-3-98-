...
make_flexible_draw <- function(ranking, participants, cup_id) {
  ...
  # Only GLOBAL ranks 1-8 are protected seeds.
  seeded <- selected_rank %>%
    filter(rank <= 8) %>%
    arrange(rank)

  unseeded <- setdiff(participants, seeded$player)
  ...
  # Each row represents one first-round match.
  slots <- tibble(
  ...
  # Preserve the old 16-player seed layout when there are 8 first-round matches:
  # Upper half = 1,4,6,7 ; Lower half = 2,3,5,8.
  preferred_seed_order <- c(1, 4, 6, 7, 2, 3, 5, 8)

  if (n_matches >= 8) {
    # Spread the 8 protected seed positions through the bracket.
    if (n_matches == 8) {
      ...
      # Guarantee eight unique positions for larger brackets.
  ...
  # Place participating global top-8 players into distinct matches.
  if (nrow(seeded) > 0) {
    ordered_seed_ranks <- preferred_seed_order[
      preferred_seed_order %in% seeded$rank
    ]

    # Include any selected seed rank not already present (safety).
    ordered_seed_ranks <- c(
      ordered_seed_ranks,
      setdiff(seeded$rank, ordered_seed_ranks)
    )

    for (i in seq_along(ordered_seed_ranks)) {
      sr <- ordered_seed_ranks[i]
      p <- seeded$player[match(sr, seeded$rank)]
  ...
  # First, fill completely empty matches with one random unseeded player.
  # This prevents an impossible BYE-vs-BYE match.
  ...
  # Put all remaining unseeded players randomly into remaining empty positions.
  if (length(unseeded) > 0) {
    ...
    # Never place an unseeded player on top of an existing player;
    # every selected slot here is genuinely empty.
  ...
  # Convert remaining single empty positions into BYEs.
  slots <- slots %>%
    mutate(
  ...
  # Safety check: no first-round match may contain two seeded players.
  bad <- slots %>%
      ...
      div(
        ...
        tags$ul(
          ...
          tags$li(
            ...
            "participating global ranks 1–8 are protected seeds; all other participants are randomly placed and BYEs are assigned automatically when the field is not a power of two."
          ),
...
server <- function(input, output, session) {
  ...
  tournament_draw <- reactiveVal(saved_draw)
  ...
  output$tournament_public <- renderUI({
    ...
    cup_name <- if ("cup_id" %in% names(d)) d$cup_id[1] else "Tournament"
    participant_count <- if ("participant_count" %in% names(d)) d$participant_count[1] else NA
    bracket_size <- if ("bracket_size" %in% names(d)) d$bracket_size[1] else NA
    round_name <- if ("round" %in% names(d)) d$round[1] else "First Round"
    ...
    draw_box <- function(r) {
      div(
        ...
        if (isTRUE(r$bye)) {
          tagList(
            tags$br(),
            tags$small(style="color:#667085;", "Automatic BYE")
          )
        }
      )
    }
    ...
    tagList(
      div(
        ...
        p(
          paste0(
            ifelse(is.na(participant_count), "", paste0(participant_count, " participants • ")),
            ifelse(is.na(bracket_size), "", paste0(bracket_size, "-slot bracket • ")),
            round_name
          )
        ),
        tags$small(
          style = "color:#667085;",
          "Protected seeds are current global ranking positions 1–8 who are participating in the cup."
        )
      ),
      ...
      fluidRow(
        column(
          6,
          h4("Upper Half"),
          lapply(seq_len(nrow(upper)), function(i) draw_box(upper[i, ]))
        ),
        column(
          6,
          h4("Lower Half"),
          lapply(seq_len(nrow(lower)), function(i) draw_box(lower[i, ]))
        )
      ),

      tags$hr(),
      p("The draw stays fixed after saving. A new draw is created only when Admin generates and saves another one.")
    )
  })
    ...
    tagList(
      ...
      tabsetPanel(
        ...
        tabPanel(
          ...
          div(
            class = "cardx",
            h3("Tournament Draw Generator"),
            p(
              "Select the players participating in this cup. ",
              "Global ranking positions 1–8 are protected seeds and cannot face each other in the first round. ",
              "All other players are placed randomly. BYEs are assigned automatically when needed."
            ),
  ...
  # ----------------------------
  # FLEXIBLE TOURNAMENT DRAW ADMIN
  # ----------------------------
  ...
  output$draw_admin_summary <- renderUI({
    ...
    selected_rank <- rankdat() %>%
      filter(player %in% input$draw_participants)

    protected <- selected_rank %>%
      filter(rank <= 8) %>%
      arrange(rank)
      ...
      tags$span(
        ...
        paste0(
          ...
          if (nrow(protected) == 0) {
            ...
            paste0(
              "#", protected$rank, " ", protected$player,
              collapse = " • "
            )
          }
        )
  ...
  output$admin_draw_table <- renderDT({
    ...
    if (is.null(d) || nrow(d) == 0) {
      return(
        datatable(
          data.frame(Message = "No draw yet."),
          rownames = FALSE,
          options = list(dom = "t")
        )
      )
    ...
    display <- d %>%
      transmute(
        ...
        `Player A` = ifelse(
          is.na(seed_a),
          player_a,
          paste0("#", seed_a, " ", player_a)
        ),
        `Player B` = ifelse(
          is.na(seed_b),
          player_b,
          paste0("#", seed_b, " ", player_b)
        ),
  ...
  observeEvent(input$save_draw, {
    ...
    tryCatch({
      ...
      showNotification(
        paste0(
          d$cup_id[1],
          " tournament draw saved to Dropbox."
        ),
        type = "message",
        duration = 6
      )
