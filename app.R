...
make_flexible_draw <- function(ranking, participants, cup_id) {
  ...
  # BYE COMPRESSION
  #
  # If two adjacent first-round matches feed the same next-round match
  # and BOTH are player-vs-BYE, there is no reason for both players to
  # receive a bye and then immediately face each other.
  #
  # Example:
  #   Match 1: Naser vs BYE
  #   Match 2: Sepehri vs BYE
  #
  # becomes:
  #   Match 1: Naser vs Sepehri
  #   Match 2: BYE vs BYE
  #
  # The renderer then shows the next round as:
  #   Winner Match 1 vs BYE
  #
  # So the actual head-to-head happens immediately, as expected.
  # --------------------------------------------------------
  feeder_pairs <- split(
    seq_len(n_matches),
    ceiling(seq_len(n_matches) / 2)
  )

  for (pair_ids in feeder_pairs) {
    if (length(pair_ids) != 2) next

    i1 <- pair_ids[1]
    i2 <- pair_ids[2]

    real1 <- c(slots$player_a[i1], slots$player_b[i1])
    real1 <- real1[real1 != "BYE"]

    real2 <- c(slots$player_a[i2], slots$player_b[i2])
    real2 <- real2[real2 != "BYE"]

    single_bye_1 <- length(real1) == 1
    single_bye_2 <- length(real2) == 1

    if (single_bye_1 && single_bye_2) {
      p1 <- real1[1]
      p2 <- real2[1]

      # Preserve seed labels if present.
      s1 <- if (slots$player_a[i1] == p1) slots$seed_a[i1] else slots$seed_b[i1]
      s2 <- if (slots$player_a[i2] == p2) slots$seed_a[i2] else slots$seed_b[i2]

      # Put both real players into the first feeder match.
      slots$player_a[i1] <- p1
      slots$seed_a[i1] <- s1
      slots$player_b[i1] <- p2
      slots$seed_b[i1] <- s2

      # The second feeder becomes an automatic empty BYE path.
      slots$player_a[i2] <- "BYE"
      slots$player_b[i2] <- "BYE"
      slots$seed_a[i2] <- NA_integer_
      slots$seed_b[i2] <- NA_integer_
    }
  }
  ...
  # Safety:
  # Protected seeds normally do not meet in the first round.
  # The only exception is deliberate BYE compression: if two seeded players
  # would both receive BYEs and then meet immediately in the next round,
  # they are allowed to play now and the paired feeder becomes BYE-vs-BYE.
  seeded_head_to_head <- which(
    !is.na(slots$seed_a) &
    !is.na(slots$seed_b)
  )

  if (length(seeded_head_to_head) > 0) {
    invalid_seed_pair <- vapply(
      seeded_head_to_head,
      function(mi) {
        paired_mi <- if (mi %% 2 == 1) mi + 1L else mi - 1L

        if (paired_mi < 1 || paired_mi > nrow(slots)) {
          return(TRUE)
        }

        paired_is_empty_bye <- (
          slots$player_a[paired_mi] == "BYE" &&
          slots$player_b[paired_mi] == "BYE"
        )
    ...
    if (any(invalid_seed_pair)) {
      stop(
        "Draw safety check failed: protected seeds were paired too early."
      )
    }
  }
