...
build_group_knockout_template <- function(n_groups, qualifiers_per_group) {
  ...
  slots <- slots %>%
    mutate(
      ...
      bracket_size = bracket_size,
      round = round_name_from_bracket(bracket_size),
      generated_at = format(Sys.time(), "%Y-%m-%d %H:%M:%S")
    ) %>%
    select(
      cup_id,
    ...
    tagList(
      div(
        ...
        p({
          ...
          paste0(
            d$n_groups[1], " groups • Group sizes: ",
      ...
      div(
        ...
        h3("Knockout Qualification Bracket"),
        p(
          "A1 = Group A winner, A2 = Group A runner-up, and so on. ",
          "The bracket shows the full path from the first knockout round to the Final."
        ),
        render_bracket_tree(ko)
      )
    )
