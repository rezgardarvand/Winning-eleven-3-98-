...
render_bracket_tree <- function(draw, results = NULL) {
  ...
  player_line <- function(
    ...
    style <- switch(
      state,
      winner = "font-weight:800;color:#067647;",
      eliminated = "text-decoration:line-through;color:#d92d20;opacity:.65;",
      normal = ""
    )
...
ui <- fluidPage(
  tags$head(
      ...
      tags$script(
        HTML(
          "window.downloadBracketHD = function() {
             var node = document.getElementById('safe-bracket-capture');
             if (!node || typeof html2canvas === 'undefined') {
               alert('Bracket export is not ready yet.');
               return;
             }

             var target = node.querySelector('.modern-bracket-wrap') || node;
             var cupSelect = document.getElementById('public_tournament_cup');
             var cup = (cupSelect && cupSelect.value) ? cupSelect.value : 'tournament';
             ...
             html2canvas(target, {
               ...
               a.download = cup + '_bracket_HD.png';
...
/* ============================================================
   SAFE LIGHT TOURNAMENT BRACKET STYLE — CSS ONLY
   ============================================================ */

#safe-bracket-capture{
  background:#f8fafc !important;
...
.mb-match{
  ...
  box-shadow:0 3px 9px rgba(16,24,40,.08) !important;
  color:#101828 !important;
}
...
/* Existing renderer marks losers with inline line-through.
   Make that line clearly red. */
.mb-player[style*="line-through"]{
...
/* Existing renderer marks current winners with green inline color.
   Make winners blue instead. */
.mb-player[style*="#067647"]{
  color:#1570ef !important;
  font-weight:900 !important;
}

/* Any real player already appearing in QF/SF is an advanced/direct qualifier.
   Highlight blue and show a blue arrow — purely visual, no R logic change. */
.mb-round2 .mb-player:not(.mb-bye):not([style*="line-through"]),
.mb-round3 .mb-player:not(.mb-bye):not([style*="line-through"]){
  color:#1570ef !important;
  font-weight:900 !important;
...
.mb-round2 .mb-player:not(.mb-bye):not([style*="line-through"])::before,
.mb-round3 .mb-player:not(.mb-bye):not([style*="line-through"])::before{
  content:"→";
  color:#1570ef;
  font-size:17px;
  font-weight:900;
...
/* Never blue-highlight placeholders. */
.mb-round2 .mb-player,
.mb-round3 .mb-player{
...
/* Final real entrants */
.mb-final .mb-player:not(.mb-bye):not([style*="line-through"]){
  color:#1570ef !important;
  font-weight:900 !important;
}

.mb-final .mb-player:not(.mb-bye):not([style*="line-through"])::before{
  content:"→";
  color:#1570ef;
  font-size:17px;
  font-weight:900;
...
.safe-bracket-legend .blue{
  color:#1570ef;
  font-weight:800;
}
.safe-bracket-legend .red{
  color:#d92d20;
  font-weight:800;
  ...
  output$tournament_public <- renderUI({
    ...
    tagList(
      ...
      div(
        class = "cardx",
        div(
          class = "safe-bracket-toolbar",
          tags$button(
            type = "button",
            class = "btn safe-download-btn",
            onclick = "downloadBracketHD();",
            "⬇ Download Bracket HD"
          )
        ),
        div(
          id = "safe-bracket-capture",
          tryCatch(
          ...
          div(
            class = "safe-bracket-legend",
            span(class = "blue", "→ Advanced / Winner"),
            span(class = "red", "Eliminated"),
            span("🏆 Champion")
          )
        )
      ),
