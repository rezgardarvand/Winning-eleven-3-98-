...
render_bracket_tree <- function(draw, results = NULL) {
  ...
  player_line <- function(
    ...
    div(
      ...
      if (!is.null(score) && !is.na(score)) {
        span(
          style = "float:right;font-weight:800;margin-left:8px;",
          score
        )
      },
      if (state == "eliminated") {
        span(
          style = "font-size:10px;margin-left:7px;text-decoration:none;opacity:.9;",
          "Eliminated"
        )
      }
    )
  ...
  match_box <- function(r, side = "left") {
    ...
    # Direct qualifier: not an actual match.
    if (xor(a == "BYE", b == "BYE")) {
      return(
        div(
          class = "mb-direct-spacer",
          style = "visibility:hidden;min-height:72px;"
        )
      )
    }
  ...
  left_col1 <- div(
    class = "mb-col",
    div(class = "mb-title", first_round_title),
    lapply(
      seq_len(nrow(left_first)),
      function(i) match_box(left_first[i, ], "left")
    )
  )

  left_col2 <- div(
    class = "mb-col mb-round2",
    div(class = "mb-title", next_round_title),
    lapply(
      left_round2,
      function(x) round_box(x, "left")
    )
  )
  ...
  right_col1 <- div(
    ...
    lapply(
      ...
      function(i) match_box(right_first[i, ], "right")
    )
  )
  ...
  center <- div(
    ...
    div(
      ...
      if (!is.na(champion) && nzchar(champion)) {
        tagList(
          span(
            style = "font-weight:900;color:#b8860b;font-size:15px;",
            paste0("🏆 ", champion)
          ),
          tags$br(),
          span("CHAMPION")
  ...
  div(
    ...
    div(
      ...
      span("Live bracket: confirmed results automatically advance winners."),
      span(
        style = "color:#1570ef;font-weight:800;",
        "→ Blue = Advanced"
      ),
      span(
        style = paste0(
...
ui <- fluidPage(
  tags$head(
      ...
      tags$script(HTML("
        window.downloadTournamentBracketHD = function() {
          var node = document.getElementById('tournament-bracket-capture');

          if (!node) {
            alert('Tournament bracket is not visible yet.');
            return;
          }

          if (typeof html2canvas === 'undefined') {
            alert('Bracket export library has not loaded yet. Please try again in a moment.');
            return;
          }

          var btn = document.getElementById('download_bracket_hd');
          var oldText = null;

          if (btn) {
            oldText = btn.innerText;
            btn.disabled = true;
            btn.innerText = 'Preparing HD image...';
          }

          var select = document.getElementById('public_tournament_cup');
          var cup = select && select.value ? select.value : 'tournament';

          // Capture the complete bracket, including horizontally scrollable content.
          var target = node.querySelector('.modern-bracket') || node;
          ...
          html2canvas(target, {
            ...
            link.download = cup + '_bracket_HD.png';
            ...
            console.error(err);
            alert('Could not export the bracket image.');
          }).finally(function() {
            if (btn) {
              btn.disabled = false;
              btn.innerText = oldText || 'Download Bracket HD';
            }
          });
        };
      ")),
...
.mb-player-advanced{
  ...
  background:rgba(21,112,239,.10) !important;
  border-left:4px solid #1570ef !important;
...
/* ============================================================
   REFERENCE BRACKET VISUAL — CSS ONLY
   Built on the user's known-working app.
   NO R / SERVER / DROPBOX / BRACKET LOGIC CHANGES.
   ============================================================ */
...
.modern-bracket{
  ...
  grid-template-columns:185px 205px 185px 175px 185px 205px 185px !important;
  gap:30px !important;
...
.mb-match{
  ...
  box-shadow:0 3px 10px rgba(16,24,40,.09) !important;
  color:#101828 !important;
}

/* Small Match / QF / SF label */
.mb-matchno{
...
/* Keep the small Eliminated label red, but NOT crossed */
.mb-player-eliminated span:last-child{
  color:#d92d20 !important;
}

/* Advanced player in the next round */
.mb-player-advanced{
...
/* A winner inside a completed card */
.mb-player[style*="font-weight:800"]{
  color:#0969da !important;
  font-weight:900 !important;
}

/* First-round left side: blue connector to next round */
.mb-left-match::after{
  content:"" !important;
  position:absolute !important;
  top:50% !important;
  ...
  border-top:2px solid #1570ef !important;
  z-index:1 !important;
}
...
.mb-left-match::before{
  ...
  border-top:2px solid #1570ef !important;
  border-right:2px solid #1570ef !important;
  transform:rotate(45deg) !important;
  z-index:2 !important;
}

/* Right side flows toward the center */
...
/* Do not draw a connector on alignment-only direct-qualifier spacers */
.mb-direct-spacer::before,
.mb-direct-spacer::after{
...
.mb-cup{
  ...
  filter:drop-shadow(0 4px 5px rgba(16,24,40,.18)) !important;
}
...
/* Status line above bracket */
.modern-bracket-wrap > div:first-child{
  color:#667085 !important;
}

/* Existing Download button — just make it reference-style blue */
#download_bracket_hd{
...
#download_bracket_hd:hover,
#download_bracket_hd:focus{
  background:#175cd3 !important;
  border-color:#175cd3 !important;
  color:#ffffff !important;
}

/* Make the outer Tournament card cleaner without changing the rest of the app */
#tournament-bracket-capture{
  background:#f8fafc !important;
  border-radius:12px !important;
}

/* Mobile remains scrollable rather than breaking the bracket */
