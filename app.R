...
# ============================================================
# PLAYER PHOTOS - AUTO MATCH + KNOWN ALIASES
# ============================================================
...
# For players whose Ranking name is different from the photo filename.
# Exact filename match is still tried first.
player_photo_alias <- c(
...
player_photo <- function(player_name) {
  ...
  if (length(hit) >= 1) {
    return(utils::URLencode(files[hit[1]], reserved = FALSE))
  }

  # 2) Known alias fallback
  alias <- unname(player_photo_alias[player_name])
  if (length(alias) == 1 && !is.na(alias) && nzchar(alias)) {
    alias_target <- normalize_photo_name(alias)
    hit <- which(stems == alias_target)
    if (length(hit) >= 1) {
      return(utils::URLencode(files[hit[1]], reserved = FALSE))
    }
  }

  # 3) No reliable match -> default avatar
  NULL
}
          ...
          fluidRow(
            ...
            column(
              ...
              div(
                class = "cardx",
                h3("Existing Players"),
                p(style="color:#667085;font-size:12px;", "Photo naming: put PlayerName.jpg/png/webp inside www. Example: Arman.jpg for player Arman."),
