...
# ============================================================
# PLAYER PHOTOS
# Only confident matches. Others get a default avatar.
# ============================================================
...
player_photo <- function(player_name) {
  # 1) Automatic lookup:
  # Any image in www/ whose filename (without extension) matches the
  # Ranking player name is used automatically, case-insensitively.
  if (dir.exists("www")) {
    files <- list.files(
      "www",
    ...
    if (length(files) > 0) {
      ...
      hit <- which(normalize_name(stems) == normalize_name(player_name))

      if (length(hit) >= 1) {
        return(utils::URLencode(files[hit[1]], reserved = TRUE))
      }
    }
  }

  # 2) Explicit mapping fallback for filenames that do not exactly
  # match the player's Ranking name.
...
load_players <- function() {
  ...
  if (!is.null(cloud)) return(normalize_players(cloud))

  if (file.exists(PLAYERS_FILE)) {
    return(normalize_players(read_csv(PLAYERS_FILE, show_col_types = FALSE)))
  }

  normalize_players(NULL)
}

load_matches <- function() {
  cloud <- dropbox_download_csv(DROPBOX_MATCHES_PATH)
  if (!is.null(cloud)) return(normalize_matches(cloud))
