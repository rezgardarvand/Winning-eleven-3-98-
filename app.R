...
save_player_pins <- function(x) {
  payload <- normalize_player_pins(x)
  dropbox_upload_csv(payload, DROPBOX_PLAYER_PINS_PATH)

  verify <- dropbox_download_csv(DROPBOX_PLAYER_PINS_PATH, quiet = FALSE)
  normalize_player_pins(verify)
}

is_valid_player_pin <- function(player, pin, pins_data) {
  if (is.null(player) || !nzchar(as.character(player))) return(FALSE)
  if (is.null(pin) || !nzchar(as.character(pin))) return(FALSE)
  ...
  identical(
    tolower(hash_password(as.character(pin))),
    tolower(as.character(row$pin_hash[1]))
  )
}
...
save_friendly_pending <- function(x) {
  payload <- normalize_friendly_pending(x)
  dropbox_upload_csv(payload, DROPBOX_FRIENDLY_PENDING_PATH)

  verify <- dropbox_download_csv(DROPBOX_FRIENDLY_PENDING_PATH, quiet = FALSE)
  normalize_friendly_pending(verify)
}
...
server <- function(input, output, session) {
  ...
  observeEvent(input$confirm_friendly_match, {
    ...
    tryCatch({
      ...
      verified_match <- verify_matches %>%
        filter(
          match_id == new_id,
          stage == "Friendly"
        )

      if (nrow(verified_match) != 1) {
        stop("Dropbox verification failed after confirming the Friendly match.")
      }

      # Remove confirmed pending row only after official match save succeeds.
